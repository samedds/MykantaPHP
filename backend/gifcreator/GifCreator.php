...
class GifCreator
{
	...
	public function create($frames = array(), $durations = array(), $loop = 0)
    {
		...
		for ($i = 0; $i < count($frames); $i++) {
			...
			/*	$stamp = imagecreatefrompng('img/mk_stamp.png');
				$marge_right = 10;
                $marge_bottom = 10;
                $sx = imagesx($stamp);
                $sy = imagesy($stamp);
                imagecopy($resourceImg, $stamp, imagesx($resourceImg) - $sx - $marge_right, imagesy($resourceImg) - $sy - $marge_bottom, 0, 0, imagesx($stamp), imagesy($stamp));
                  */ob_start();
