...
function chatbusinessClick(shop_id,client_id)
{
	//alert(friend_id);
	$('#chatWrapper').show();
	$('#hiddenId').val(shop_id);
	$('#hiddenFriendId').val(client_id);
	$('.chat-container').removeClass("open");
	//$('#chatTitle').html("sup");
	//loadLog_business();
	//load the chat contents instantly
	//setInterval(loadLog_business, 10000);	//Reload file every 2500 ms or x ms if you w
...
function chatViewClick(id,friend_id,counter)
{
	...
	chatbusinessClick(id,friend_id);
