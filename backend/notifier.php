...
$query = "SELECT * FROM registration where id = $account_id  LIMIT 10 ";
	...
	while($details = mysqli_fetch_assoc($query_add))
		...
		echo '<a href="/connection_auth/'.$username.'"  class="" style="font-style:none;">'.$username.'  </a>';
		}
	}
}*/  
...
///////////////////////////////////		
function get_title_post_here($post_id)
{include "conxn.php";
...
FROM account_comment WHERE owner_id = account_id AND owner_id IN $range /*AND datetime > '$lastOut'*/ order by commentDate desc LIMIT 3";
...
FROM friends WHERE value = '0' AND account_id='$thisUser'  AND friend_id IN $range_0  /*AND datetime > '$lastOut'*/ order by datetime desc LIMIT 3";  
...
FROM subscribers WHERE user_id IN $range /*AND datetime > '$lastOut'*/ order by datetime desc LIMIT 3";  
...
FROM friends WHERE value = '1' AND friend_id IN $range  OR account_id IN $range  AND  value = '1' /*AND datetime > '$lastOut'*/ order by datetime desc LIMIT 3";  
...
FROM reply WHERE  post_user_id ='$thisUser'  AND reply_user_id !='$thisUser' order by datetime desc LIMIT 3";   
 ...
 $owner_id_new = get_last_reev_owner($friend_id) ;
  ...
  echo '<li class="  row no-padding" style="margin:7px; background-color:rgb(221, 221, 221); border-radius:10px;">
				<div class="col-xs-12 col-md-12 padding-5 ">
					 <img src="',get_profile_pic_only_name($friend_id_new),'"class=" img-circle" width="25px;" height="auto;"/>
...
{
	  if( $datetime_lastOut - $datetime0 < 2000){ 
	  echo '<li class="  row  no-padding" style="margin:7px;  background-color:rgb(221, 221, 221); border-radius:10px;">
				<div class="col-xs-12 col-md-12 padding-5 ">
					 <img src="',get_profile_pic_only_name($friend_id),'"class=" img-circle" width="20px;" height="auto;"/>
...
else if($comment == 'newfriend')
{
	  ...
	  echo '<li class="  row no-padding" style="margin:7px;  background-color:rgb(221, 221, 221); border-radius:10px;">
				<div class="col-xs-12 col-md-12 padding-5">
							 <img src="',get_profile_pic_only_name($account_id),'"class=" img-circle" width="20px;" height="auto;"/>
				 <span><strong class="text-muted" style="color:grey; font-size:0.9em;">',get_status_name($account_id),' </strong>	<span class="text-muted" style="font-size:0.9em; bottom:0px;">  is now in connection with </span><strong class="text-muted" style="color:grey; font-size:0.9em;"> <span class="" style="width:20px;"> <img src="',get_profile_pic_only_name($friend_id),'"class=" img-circle" width="20px;" height="auto;"/> </span>',get_status_name($friend_id),' </strong></span>
					<small class="pull-right text-muted" style="font-size:0.8em; bottom:0px;" > '.$datetimemonth.' '.$datetimeday.'</small>
			 	</div>
			 </li> 
';
...
else if($comment == 'newcomment')
{
 ...
 if($owner_id_new != '0' ){
	...
	echo '<li class="row padding-5" style="margin:7px; background-color:rgb(221, 221, 221); border-radius:10px;">
				<div class="col-xs-12 col-md-12 no-padding ">
						 <img src="',get_profile_pic_only_name($account_id),'"class=" img-circle" width="25px;" height="auto;"/>
