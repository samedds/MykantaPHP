...
$range = '(' . implode(",",$all_shops) . ')';
	  //now call products from businesses
		  $query1 = "SELECT product_id,shop_id,name,account_id,price,descrb,image_loc,mode
			 FROM product
			 WHERE mode >= '1' AND shop_id IN $range order by RAND() ASC 
			 LIMIT 0 , 50
			 ";
...
<div  style="">
 ...
 <a class=" " style="font-size:1.0em; color:black; " title="'.$prduct_name.'" href="/product/'.$shopName.'/'. $product_id .'/'.$prdt_name_format.'">'.$prdt_name_ok.'</a>
...
',product_option($shop_id,$product_id),' <a class="text-info pull-right" href="/product/'.$shopName.'/'. $product_id .'/'.$prdt_name_format.'">view page</a>
</div>
</div>
</div>
