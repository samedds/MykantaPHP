...
if(isset($_POST['product_id']) && isset($_POST['task']))
{ include "include/conxn.php";
...
&times;
	 ...
	 <a href="/product-link/'.strtolower($shopNameformat).'/'.$product_id .'/'.strtolower($prdt_name_format).'">
...
$shopName = $_POST['shopName'];		//new
$shopNameformat= formatUrl($shopName);
 $shop_id = safe_input($_POST['shop_id']);			//new
...
$query = "
SELECT * FROM product where product_id = $product_id LIMIT 30
";
if($query_add = mysqli_query($link,$query))
	{
	while($products = mysqli_fetch_assoc($query_add))
