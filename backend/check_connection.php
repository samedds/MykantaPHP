...
{
$friend_id = $query_count['id'];

$query = "SELECT * FROM friends WHERE account_id = '$friend_id' AND friend_id = '$account_id' and value >='0'";
$query_q = mysqli_query($link, $query);
if(mysqli_num_rows($query_q) != 1) 
  {
	$queryl = "SELECT * FROM friends WHERE friend_id = '$friend_id' AND account_id = '$account_id' and value >='0'";
	$query_3 = mysqli_query($link, $queryl);
	if(mysqli_num_rows($query_3) != 1) 
	  {
		$queryl = "SELECT * FROM friends WHERE account_id = '$friend_id' AND friend_id = '$account_id' and value >='0'";
		$query_3 = mysqli_query($link, $queryl);
		if(mysqli_num_rows($query_3) != 1) 
		  {
			 Destroy_user();
